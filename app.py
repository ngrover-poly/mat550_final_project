...
def load_data():
    ...
    openings = pd.read_csv('job_openings.csv', index_col=0, parse_dates=True)
    hires = pd.read_csv('hires.csv', index_col=0, parse_dates=True)
    quits = pd.read_csv('quits.csv', index_col=0, parse_dates=True)
    
    df = pd.concat([openings, hires, quits], axis=1)
    df.columns = ['OpeningsRate', 'HiresRate', 'QuitsRate']
